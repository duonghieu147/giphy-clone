...
export class GifsService {
  ...
  getTrendGifs(api_key: string = GIPHY_FE_WEB_API_KEY): Observable<any> {
    return this.http.get<Config>("https://api.giphy.com/v1/gifs/trending?api_key="+api_key)
  ...
  getSearchGifs(api_key: string = GIPHY_FE_WEB_API_KEY,keyWord:string): Observable<any> {
    return this.http.get<Config>("https://api.giphy.com/v1/gifs/search?offset=0&type=gifs&sort=&q="+keyWord+"&api_key="+api_key)
      .pipe(catchError(this.handleError))
  }
  ...
  getRelatedGifs(api_key: string = GIPHY_FE_WEB_API_KEY,gif_id:string): Observable<any> {
    return this.http.get<Config>("https://api.giphy.com/v1/gifs/related?gif_id="+gif_id+"&api_key="+api_key)
      .pipe(catchError(this.handleError))
  }

  //TO DO
  getTrendStories(api_key: string = GIPHY_FE_STORIES_AND_GIPHY_TV_API_KEY): Observable<any> {
    return this.http.get<Config>("https://x.giphy.com/v1/stories/trending?api_key="+api_key+"&sort=desc&gif_hydration_method=COVER_ONLY")
      .pipe(catchError(this.handleError))
  }

  getTrendVideos(api_key: string = GIPHY_FE_WEB_API_KEY): Observable<any> {
    return this.http.get<Config>("https://api.giphy.com/v1/videos/trending?api_key="+api_key+"&offset=0")
      .pipe(catchError(this.handleError))
  }

  getGiphy(): Observable<any> {
    return this.http.get<Config>("https://giphy.com/")
      .pipe(catchError(this.handleError))
  }
}
